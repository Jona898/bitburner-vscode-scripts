...
export async function main(ns) {
  ...
  function log() {
    ...
    for (let t of tmp) {
      ns.print(`║ ${act[t[1]]} ║ ${str(t[1])}` + `${ns.formatNumber(info('MA', t[1]), 0)} / ${ns.formatNumber(info('MM', t[1]), 0)} : ${ns.formatPercent(info('MA', t[1]) / info('MM', t[1]), 0)} ║`.padStart(36 - str(t[1]).length))
    }
    ns.print('╠═══╩════════════════════════════════════╝')
