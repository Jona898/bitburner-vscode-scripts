...
export async function main(ns) {
  ...
  const MULTIPLIER_MONEY = dataFlags.mu > 1.05 ? dataFlags.mu : 2
  ...
  /** 
   * @param { keyof scripts } type
   */
  function getScriptBody(type) {
  ...
  for (let type in scripts) {
    scripts[type].scriptRam = ns.getScriptRam(scripts[type].filename)
  }
  ...
  // // /** @type { {[key: string]: ReturnType<getServerForHacking>} } */
  /** @type { Map< string, ReturnType<getServerForHacking> > } */
  let targets = new Map()
  /** @type { Set<string> } */
  let weakeningTargets_Set = new Set()
  /** @type { string[] } */
  let weakenedTargets = []
  ...
  function updateAllTargets() {
    let hackingLevelTresh = Math.max(ns.getHackingLevel() * 0.6, 1)
  ...
  /** @param { number } bestElementCount */
  function getBestTargets(bestElementCount = 15) {
    // @todo edit this part
    for (let server of targets.values()) {
  ...
  /**
   * @param { ReturnType<getServerForHacking> } server
   */
   ...
   * @param { ReturnType<getServerForHacking> } server
   */
  function calculateServerValue(server) {
    let hackingLevelTresh = ns.getHackingLevel() * 0.6
  ...
  /** @param { string } hostname 
   * @return All needed Values to evaluate Server
   */
  function getServerForHacking(hostname) {
    let target = {
      hostname,
  ...
  /**
   * @param { string } target
   * @returns true if weaken could be started
   */
  function prepareTargetForHack(target) {
    ...
    let growthThreads = multiplier > 1
      ? multiplier < Infinity
        ? Math.ceil(ns.growthAnalyze(target, multiplier, 1) * 1.1)
        : 500000
      : 0
    let additionalSecurityGrowth = Math.ceil(ns.growthAnalyzeSecurity(growthThreads, target, 1))
    const weakenAmmount1Thread = ns.weakenAnalyze(1, 1)
    let weakenThreadsToMin = Math.ceil((ns.getServerSecurityLevel(target) - ns.getServerMinSecurityLevel(target)) / weakenAmmount1Thread)
    let weakenThreadsForGrowth = Math.ceil((additionalSecurityGrowth * 1.1) / weakenAmmount1Thread)


    let weakenStarted = startProcesOnFreeMomory("weakOnce", target, 0, weakenThreadsToMin) == 0

    let multiplierPartProcersses = Math.min(getTotalFreeRam() / (growthThreads * scripts.growLoop.scriptRam + weakenThreadsForGrowth * scripts.weakLoop.scriptRam), 1)
  ...
  /**
   * @param { ReturnType<getServerForHacking> } target
   * @param { number } percentUseRam
   */
  function ratioAttack(target, percentUseRam) {
    if (0 >= percentUseRam || percentUseRam > 1) throw "Percent use Ram has to be between 0 and 1"

    let ramToUse = getTotalFreeRam() * percentUseRam
    ...
    let ramOneRatio = totalWeakenThreads * scripts.weakLoop.scriptRam
      + totalGrowthThreads * scripts.growLoop.scriptRam
      + totalHackThreads * scripts.hackLoop.scriptRam

    let timesUseAttack = ramToUse / ramOneRatio

    totalWeakenThreads *= timesUseAttack
    totalGrowthThreads *= timesUseAttack
    totalHackThreads *= timesUseAttack

    let totalUsedRam = totalWeakenThreads * scripts.weakLoop.scriptRam
      + totalGrowthThreads * scripts.growLoop.scriptRam
      + totalHackThreads * scripts.hackLoop.scriptRam

    const alert_str = `${target.hostname}: Use ${ns.formatRam(ramToUse, 1)}/${ns.formatRam(totalUsedRam, 1)} with ${Math.floor(totalWeakenThreads)}w, ${Math.floor(totalGrowthThreads)}g, ${Math.floor(totalHackThreads)}h`
    ns.tprint(alert_str)
    ns.print(alert_str)
  ...
  let processId = 1
  /** 
   * @param { keyof scripts } type what Script to deploy
   * @param { string } target Server to attack
   * @param { number } additionalWaitTime Time to wait bivore first run
   * @param { number } _threadcount Number of threads to start in total
   * @param { number } batchthreadcount number of threads per start
   * @returns Thread count function couldn't start
   */
  function startProcesOnFreeMomory(type, target, totalThreadcount, additionalWaitTime = 0, batchthreadcount = Infinity) {
  ...
  while (true) {
    ...
    /** @type { ReturnType< getBestTargets > } */
    let bestTargetServers
    ...
    do {
      ...
      for (let i = 0; i < 3; i++) {
        if (bestTargetServers[i]?.hostname) {
          ...
          if (!prepareTargetForHack(bestTargetServers[i].hostname)) {
            ...
            ns.asleep(Math.max(ns.getGrowTime(bestTargetServers[i].hostname), ns.getWeakenTime(bestTargetServers[i].hostname)) + 100)
              .then(() => {
